        ...
        afterDatasetsDraw: function (chart) {
          ...
          if (yAxis === "PenetrationPower") {
            ...
            for (let i = 1; i <= 10; i++) {
              ...
              if (
                yPosition >= chartOffset &&
                yPosition <= chartOffset + chartHeight
              ) {
...
function updateChart() {
  ...
  selectedCalibers.forEach((caliber) => {
    const caliberData = data.item[caliber];
    if (caliberData) {
  ...
  if (useDynamicScaling) {
    ...
    [minX, maxX] = data.info[xAxis] || [0, 100];
    [minY, maxY] = data.info[yAxis] || [0, 100];
  }

  ammoChart.options.scales.x.min = minX - Math.ceil((maxX - minX) * 0.05);
  ammoChart.options.scales.x.max = maxX + Math.ceil((maxX - minX) * 0.05);
  ammoChart.options.scales.y.min = minY - Math.ceil((maxY - minY) * 0.05);
  ammoChart.options.scales.y.max = maxY + Math.ceil((maxY - minY) * 0.05);
...
function createCaliberButtons() {
  ...
  Object.keys(data.item).forEach((caliber, index) => {
    const colorIndex = getIndex(caliber, colorMap);
    const symbolIndex = getIndex(caliber, symbolMap);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("dynamic-scaling-toggle")
    .addEventListener("change", function () {
